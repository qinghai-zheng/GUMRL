...
def cluster(n_clusters, features, labels, count=10):
    ...
    precision_avg, precision_std = precision_avg_f1(gt, pred_all, count)
    return acc_avg, acc_std, nmi_avg, nmi_std, ri_avg, ri_std, f1_avg, f1_std, precision_avg, precision_std
...
def precision_avg_f1(y_true, y_pred, count):
    precision_array = np.zeros(count)
    for i in range(count):
        precision_array[i] = metrics.b3_precision_score(y_true, y_pred[i])
    f1_avg = precision_array.mean()
    f1_std = precision_array.std()
    return f1_avg, f1_std
